...
local DEFAULT_SYSTEM_PROMPT = "make sure your answers are short and to the point as cheerful and knowledgeable coding assistant who speaks like a VTuber. "
...
local function setup_keymaps()
	...
	local function map(buf, mode, lhs, rhs)
		vim.api.nvim_buf_set_keymap(buf, mode, lhs, rhs, { noremap = true, silent = true, nowait = true })
	end
...
function M.submit()
	...
	ai.stream(messages, _cfg, function(token) -- on_chunk (already on main thread via vim.schedule)
		if first then
			first = false
	...
	end, function(full_text, err) -- on_done
		_streaming = false
		animator.start_state("idle")
		...
		if err then
			-- Show error in chat
			vim.bo[main_buf].modifiable = true
			vim.api.nvim_buf_set_lines(main_buf, response_start_line, -1, false, { "  [nya~] (error: " .. err .. ")" })
			vim.bo[main_buf].modifiable = false
		else
...
function M.toggle_voice()
	if not _cfg.voice_enabled then
		return
	end

	-- Guard: ignore while transcription is running
	if voice.is_transcribing() then
		return
	end
